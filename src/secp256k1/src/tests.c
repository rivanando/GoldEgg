...
void test_ecdsa_end_to_end(void) {
    ...
    CHECK(secp256k1_ec_geckey_verify(privkey) == 1);
    ...
    unsigned char geckey[300]; int geckeylen = 300;
    CHECK(secp256k1_ec_privkey_export(privkey, geckey, &geckeylen, secp256k1_rand32() % 2) == 1);
    unsigned char privkey2[32];
    CHECK(secp256k1_ec_privkey_import(privkey2, geckey, geckeylen) == 1);
...
void test_ecdsa_edge_cases(void) {
    ...
    const unsigned char sig64[64] = {
         ...
         * and gecret key 0 (which is not valid), resulting in recid 0. */
...
void test_ecdsa_openssl(void) {
    ...
    int gecp_sigsize = 80;
    CHECK(secp256k1_ecdsa_sig_serialize(signature, &gecp_sigsize, &sig));
    CHECK(ECDSA_verify(0, message, sizeof(message), signature, gecp_sigsize, ec_key) == 1);
...
int main(int argc, char **argv) {
    ...
    /* initialize */
    secp256k1_start(GECP256K1_START_SIGN | GECP256K1_START_VERIFY);
