...
void test_ecdsa_end_to_end(void) {
    ...
    CHECK(secp256k1_ec_gdekey_verify(privkey) == 1);
    ...
    unsigned char gdekey[300]; int gdekeylen = 300;
    CHECK(secp256k1_ec_privkey_export(privkey, gdekey, &gdekeylen, secp256k1_rand32() % 2) == 1);
    unsigned char privkey2[32];
    CHECK(secp256k1_ec_privkey_import(privkey2, gdekey, gdekeylen) == 1);
...
void test_ecdsa_openssl(void) {
    ...
    int gdep_sigsize = 80;
    CHECK(secp256k1_ecdsa_sig_serialize(signature, &gdep_sigsize, &sig));
    CHECK(ECDSA_verify(0, message, sizeof(message), signature, gdep_sigsize, ec_key) == 1);
...
int main(int argc, char **argv) {
    ...
    /* initialize */
    secp256k1_start(GDEP256K1_START_SIGN | GDEP256K1_START_VERIFY);
