...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("GoldeggMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("GoldeggMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("GoldeggMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("goldegg-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running GoldeggMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
