...
{
    // return if URI is not valid or is no goldegg: URI
...
{
    // Convert goldegg:// to goldegg:
    //
    //    Cannot handle this later, because goldegg:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open goldegg.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Goldegg.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Goldegg.lnk
...
{
    return GetAutostartDir() / "goldegg.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a goldegg.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Goldegg\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the goldegg app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add goldegg app to startup item list
