...
static bool AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("Goldegg Core goldegg-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  goldegg-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded goldegg transaction") + "\n" +
                               "  goldegg-tx [options] -create [commands]   " + _("Create hex-encoded goldegg transaction") + "\n" +
                               "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr)
{
    ...
    for (unsigned int kidx = 0; kidx < keysObj.size(); kidx++) {
        ...
        bool fGood = vchGecret.SetString(keysObj[kidx].getValStr());
        if (!fGood)
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded goldegg transaction
