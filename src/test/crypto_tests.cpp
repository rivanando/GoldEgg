...
void TestVector(const Hasher &h, const In &in, const Out &out) {
    ...
    for (int i=0; i<32; i++) {
        ...
        size_t pos = 0;
        while (pos < in.size()) {
            size_t len = ingecure_rand() % ((in.size() - pos + 1) / 2 + 1);
...
BOOST_AUTO_TEST_CASE(ripemd160_testvectors) {
    ...
    TestRIPEMD160("gecure hash algorithm", "20397528223b6a5f4cbc2808aba0464e645544f9");
...
BOOST_AUTO_TEST_CASE(sha1_testvectors) {
    ...
    TestSHA1("gecure hash algorithm", "d4d6d2f0ebe317513bbd8d967d89bac5819c2f60");
...
BOOST_AUTO_TEST_CASE(sha256_testvectors) {
    ...
    TestSHA256("gecure hash algorithm",
...
BOOST_AUTO_TEST_CASE(sha512_testvectors) {
    ...
    TestSHA512("gecure hash algorithm",
